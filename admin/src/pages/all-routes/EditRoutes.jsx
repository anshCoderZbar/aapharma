...
import { GetSingleRoutesMutation } from "rest/allRoutes";
import { ComponentLoader } from "components/Loader/ComponentLoader";
import { ErrorComponent } from "components/Alerts/Error";
import { EditRoutesMutation } from "rest/allRoutes";
...
export default function EditRoutes() {
  ...
  const singleRoutes = GetSingleRoutesMutation(formData);

  const editRoutes = EditRoutesMutation();
  ...
  useEffect(() => {
    ...
    const defaultInputs =
      singleRoutes?.data?.data?.subMenuArray &&
      singleRoutes?.data?.data?.subMenuArray?.map((elm, i) => ({
        subMenu: elm?.subMenu,
        slug: elm?.slug,
      }));
    ...
    defaultInputs?.map((elm, i) => {
      defaultValues[`subMenu_${i + 1}`] = elm?.subMenu;
      defaultValues[`slug_${i + 1}`] = elm?.slug;
    });

    setDefaultImg(singleRoutes?.data?.data?.Icon);
    reset(defaultValues);
  }, [singleRoutes?.data?.data]);
